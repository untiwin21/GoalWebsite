...
const Weekly = ({ data, updateData }) => {
  ...
  useEffect(() => {
    ...
    // 부모 컴포넌트(App.js)에서 내려온 데이터 확인
    if (data.todayTodosDate !== today) {
      // 날짜가 다르면 '오늘 할 일' 목록을 초기화하고 부모 상태 업데이트
      updateData({
        ...data,
        todayTodos: [],
      ...
      // 날짜가 같으면 부모 상태의 '오늘 할 일' 목록을 현재 컴포넌트 상태에 반영
      setTodayTodos(data.todayTodos || []);
    }
    setLastCheckDate(today);

    // 자정마다 목록을 초기화하기 위해 1분 간격으로 날짜 변경을 체크
